...
function toMarker(place: NearbyPlaceResult, index: number): StopsMapMarker {
  return { id: place.id, name: place.name, coordinates: place.coordinates, number: index + 1, bg: MARKER_BG, text: MARKER_TEXT }
}
 ...
 * máquina de estados de resize de RouteView.tsx). Mapa real de Mapbox GL (StopsMapView, mismo
 * componente que DIAS/RUTA — ya no la imagen estática de MapPlaceholder). Nombre/ubicación de cada
 * resultado son REALES vía Mapbox (mismo proveedor que el resto de la app, ver
 * nearbyPlacesSearch.ts); valoración/reseñas son mock — ninguna API ya integrada en la app expone
 * esos datos.
 */
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-bg md:flex-row">
      <div className="relative h-[45vh] shrink-0 md:h-auto md:flex-1">
        <StopsMapView markers={(results ?? []).map(toMarker)} activeStopId={activeId} onSelectStop={setActiveId} />
        <button
          type="button"
          onClick={onClose}
