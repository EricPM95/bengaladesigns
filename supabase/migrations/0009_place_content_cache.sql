...
-- y POI_CONTENT_SYSTEM_PROMPT en server/index.js. A diferencia de tips_anclas/zona_restaurantes
-- (ligados a viaje/franja), esto cachea por lugar+destino de forma permanente: cuanto más popular
-- el destino, más rápido se cubre su catálogo turístico y menos coste por viajero nuevo (efecto
-- progresivo). Mismo patrón de RLS que el resto de cachés globales: server/index.js se conecta con
-- la clave anónima, así que "anon" también necesita acceso.
--
-- Nota: la columna se llama "place_data" (no "content") para evitar cualquier ambigüedad con la
-- palabra reservada/identificador especial `content` en algunos contextos de PostgreSQL.
